...
MLFLOW_TRACKING_URI = "http://localhost:8080"   # match src/train.py
EXPERIMENT_NAME     = "DemandCast"

DATA_PATH   = Path(__file__).parent.parent / "data" / "features.parquet"
TEST_CUTOFF = "2025-02-01"   # CV runs on train+val; sealed test set excluded
TARGET      = "demand"
...
def time_series_cv(
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        for fold, (train_idx, test_idx) in enumerate(tscv.split(X)):
            ...
            # Log per-fold metrics with the fold number as the step
            mlflow.log_metric("fold_mae",  fold_metrics["mae"],  step=fold)
            mlflow.log_metric("fold_rmse", fold_metrics["rmse"], step=fold)
            mlflow.log_metric("fold_r2",   fold_metrics["r2"],   step=fold)
